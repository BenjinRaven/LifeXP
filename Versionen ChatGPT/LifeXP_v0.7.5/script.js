...
const elements = {
    ...
    statsHeaderTotalXp: document.getElementById("statsHeaderTotalXp"),
    weekTotalXp: document.getElementById("weekTotalXp"),
    weekAverageXp: document.getElementById("weekAverageXp"),
    ...
    themeOcean: document.getElementById("themeOcean"),
    themeForestNight: document.getElementById("themeForestNight"),
    themeObsidian: document.getElementById("themeObsidian"),
    themeBordeaux: document.getElementById("themeBordeaux"),
    themeMoonlight: document.getElementById("themeMoonlight"),
...
const BUILT_IN_THEME_PALETTES = {
    ...
    midnight: { background: "#070b12", surface: "#111827", accent: "#4f8cff" },
    purple: { background: "#0d0a12", surface: "#1a1324", accent: "#d05cff" },
    ocean: { background: "#061416", surface: "#0d2428", accent: "#2dd4bf" },
    forestNight: { background: "#08130d", surface: "#132219", accent: "#63e6a2" },
    obsidian: { background: "#090a0c", surface: "#17191d", accent: "#c4ccd8" },
    bordeaux: { background: "#170b0f", surface: "#2a151c", accent: "#f0c6b7" },
    moonlight: { background: "#0b1220", surface: "#182235", accent: "#9fd8ff" }
};
...
function renderHistory() {
    const entries = getEntryTargetHistory()
        .slice()
        .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

    const retroMode = isRetroEntryMode();

    if (elements.historySectionLabel) {
        elements.historySectionLabel.textContent = retroMode
            ? `Verlauf am ${formatRetroTitle(getEntryTargetDayKey())}`
            : "Heutiger Verlauf";
    }

    if (elements.historyTitle) {
        elements.historyTitle.textContent = retroMode
            ? "Historie dieses Tages"
            : "Historie";
    }
...
function renderStats() {
    ...
    const elapsedWeekValues = weekDays
        .filter((date) => compareDateKeys(toDateKey(date), todayKey) <= 0)
        .map((date) => getDayXp(toDateKey(date)));

    const weekTotal = elapsedWeekValues.reduce((sum, value) => sum + value, 0);
    const weekAverage = elapsedWeekValues.length
        ? Math.round(weekTotal / elapsedWeekValues.length)
        : 0;

    if (elements.statsHeaderTotalXp) {
        elements.statsHeaderTotalXp.textContent = `${totalXp} XP`;
    }

    if (elements.weekTotalXp) {
        elements.weekTotalXp.textContent = `${weekTotal} XP`;
    }

    if (elements.weekAverageXp) {
        elements.weekAverageXp.textContent = `${weekAverage} XP`;
    }

    elements.statsTodayXp.textContent = `${getTodayXp()} XP`;
    elements.statsTotalXp.textContent = `${totalXp} XP`;
...
function createManageItem(task, active) {
    ...
    if (!active) {
        ...
        activateButton.className = "small-button activate-button";
        activateButton.textContent = "+ Aktivieren";
        activateButton.addEventListener("click", () => toggleTask(task.id));

        const deleteButton = document.createElement("button");
        deleteButton.type = "button";
        deleteButton.className = "small-button delete-small-button desktop-only-action";
        ...
        hideButton.className = "small-button muted-button desktop-only-action";
    ...
    setupManageSwipe(item, task.id, active);

    return item;
}


function resetManageSwipeVisual(item) {
    item.classList.remove("swiping-action");
    item.style.removeProperty("--swipe-x");
    item.style.removeProperty("--swipe-progress");
    item.style.removeProperty("--swipe-red");
...
function setupManageSwipe(item, taskId, active) {
    let startX = 0;
    let startY = 0;
...
function beginLiveCardDrag(item, clientX, clientY, placeholderClass) {
    ...
    placeholder.className = `live-drag-placeholder ${placeholderClass}`;
    ...
    item.classList.add("live-floating-card");
    ...
    moveLiveCardDrag(clientX, clientY);

    return liveDragState;
}

function moveLiveCardDrag(clientX, clientY) {
    if (!liveDragState) return;

    const { item, offsetX, offsetY } = liveDragState;
    item.style.left = `${clientX - offsetX}px`;
    item.style.top = `${clientY - offsetY}px`;
}

function getLiveDragPlaceholder() {
    return liveDragState?.placeholder || null;
}

function endLiveCardDrag({ restore = true } = {}) {
    if (!liveDragState) return;

    const { item, placeholder } = liveDragState;
    ...
    if (item) {
        item.classList.remove("live-floating-card", "dragging");
...
function movePlaceholderBeforeOrAfter(target, clientY, placeholder) {
...
let todayVisualDragModel = null;

function beginTodayVisualDragModel(taskId, pointerX, pointerY) {
    if (!elements.todayTaskGrid) return;
    ...
    todayVisualDragModel = {
        ...
        pointerOffsetX: pointerX - draggedRect.left,
        pointerOffsetY: pointerY - draggedRect.top,
        tileWidth: draggedRect.width,
        tileHeight: draggedRect.height,
        lastSwitchAt: 0,
        lastSwitchPointerX: pointerX,
        lastSwitchPointerY: pointerY
    };
}

function clearTodayVisualDragModel() {
    todayVisualDragModel = null;
}

function getTodayDraggedCenter(pointerX, pointerY) {
    ...
    if (!todayVisualDragModel) return null;

    return {
        x:
            pointerX -
            todayVisualDragModel.pointerOffsetX +
            todayVisualDragModel.tileWidth / 2,
        y:
            pointerY -
            todayVisualDragModel.pointerOffsetY +
            todayVisualDragModel.tileHeight / 2
    };
}

function placeTodayDraggedNodeAtIndex(node, desiredIndex) {
    ...
    const safeIndex = Math.max(0, Math.min(desiredIndex, activeNodes.length));

    if (safeIndex >= activeNodes.length) {
        const addTile = elements.todayTaskGrid.querySelector(".add-task-tile");
        if (addTile) {
            elements.todayTaskGrid.insertBefore(node, addTile);
        } else {
    ...
    elements.todayTaskGrid.insertBefore(node, activeNodes[safeIndex]);
}

function updateTodayVisualSlot(node, pointerX, pointerY) {
    const model = todayVisualDragModel;
    if (!model || !node) return;

    const center = getTodayDraggedCenter(pointerX, pointerY);
    if (!center) return;

    // Etwas weniger sensibel als v0.7.4:
    // Ein Slot-Wechsel erfolgt jetzt nach ca. 36 % des Weges.
    // Zusätzlich verhindert eine kleine Hysterese nervöses Hin-und-her-Flackern.
    const ratio = 0.36;
    const now = performance.now();
    const cooldownMs = 115;
    const minTravelAfterSwitch = 16;

    const travelledSinceSwitch = Math.hypot(
        pointerX - model.lastSwitchPointerX,
        pointerY - model.lastSwitchPointerY
    );

    // Direkt nach einem Slot-Wechsel darf nicht sofort wieder zurückgeschaltet
    // werden. Erst etwas Zeit + etwas echte Finger/Maus-Bewegung später.
    if (
        model.lastSwitchAt &&
        (
            now - model.lastSwitchAt < cooldownMs ||
            travelledSinceSwitch < minTravelAfterSwitch
        )
    ) {
        return;
    }
    ...
    if (model.currentCol > 0) {
        const here = model.xCenters[model.currentCol];
        const left = model.xCenters[model.currentCol - 1];
        const threshold = here - (here - left) * ratio;
    ...
    if (
        nextCol === model.currentCol &&
        model.currentCol < model.cols - 1
    ) {
        const here = model.xCenters[model.currentCol];
        const right = model.xCenters[model.currentCol + 1];
        const threshold = here + (right - here) * ratio;
    ...
    if (model.currentRow > 0) {
        const here = model.yCenters[model.currentRow];
        const above = model.yCenters[model.currentRow - 1];
        const threshold = here - (here - above) * ratio;
    ...
    if (
        nextRow === model.currentRow &&
        model.currentRow < model.rows - 1
    ) {
        const here = model.yCenters[model.currentRow];
        const below = model.yCenters[model.currentRow + 1];
        const threshold = here + (below - here) * ratio;
    ...
    // Bei unvollständiger letzter Reihe nur existierende Slots zulassen.
    desiredIndex = Math.max(0, Math.min(desiredIndex, model.total - 1));

    if (desiredIndex === model.currentIndex) return;

    placeTodayDraggedNodeAtIndex(node, desiredIndex);
    ...
    model.lastSwitchPointerX = pointerX;
    model.lastSwitchPointerY = pointerY;
}
...
function setupManageDragHandle(handle, item, taskId) {
    ...
    item.addEventListener("dragend", () => {
        ...
        // Wenn der Drop bereits in "Inaktiv" verarbeitet wurde, wurde
    ...
    item.addEventListener("touchstart", (event) => {
        ...
        holdTimer = setTimeout(() => {
            ...
            beginLiveCardDrag(item, startX, startY, "manage-placeholder");
        }, 180);
    }, { passive: true });
    ...
    const finishTouchDrag = () => {
        ...
        if (shouldDeactivate) {
            endLiveCardDrag({ restore: false });
            finishDeactivateDrop(taskToFinish);
        } else {
...
function setupTodayDrag(item, handle, taskId) {
    item.addEventListener("dragstart", (event) => {
        ...
        beginTodayVisualDragModel(taskId, event.clientX, event.clientY);
        item.classList.add("dragging");
    ...
    item.addEventListener("touchstart", (event) => {
        ...
        holdTimer = setTimeout(() => {
            ...
            touchDragStarted = true;
            item.dataset.justDragged = "true";
            beginTodayVisualDragModel(taskId, startX, startY);
            item.classList.add("dragging");
            document.body.classList.add("live-reordering");
    ...
    const finishTouchDrag = () => {
        ...
        const draggedItemReference = touchDraggedTodayItem;
        setTimeout(() => {
            if (draggedItemReference) {
                draggedItemReference.dataset.justDragged = "false";
            }
        }, 120);
...
function renderSettings() {
    ...
    if (elements.themeMidnight) elements.themeMidnight.checked = state.theme === "midnight";
    if (elements.themePurple) elements.themePurple.checked = state.theme === "purple";
    if (elements.themeOcean) elements.themeOcean.checked = state.theme === "ocean";
    if (elements.themeForestNight) elements.themeForestNight.checked = state.theme === "forestNight";
    if (elements.themeObsidian) elements.themeObsidian.checked = state.theme === "obsidian";
    if (elements.themeBordeaux) elements.themeBordeaux.checked = state.theme === "bordeaux";
    if (elements.themeMoonlight) elements.themeMoonlight.checked = state.theme === "moonlight";
...
function openTaskDialog(taskId = "") {
    ...
    if (taskId) {
        ...
        elements.deleteCustomTaskButton.hidden = false;
    } else {
...
if (elements.themeMidnight) elements.themeMidnight.addEventListener("change", () => setTheme("midnight"));
if (elements.themePurple) elements.themePurple.addEventListener("change", () => setTheme("purple"));
if (elements.themeOcean) elements.themeOcean.addEventListener("change", () => setTheme("ocean"));
if (elements.themeForestNight) elements.themeForestNight.addEventListener("change", () => setTheme("forestNight"));
if (elements.themeObsidian) elements.themeObsidian.addEventListener("change", () => setTheme("obsidian"));
if (elements.themeBordeaux) elements.themeBordeaux.addEventListener("change", () => setTheme("bordeaux"));
if (elements.themeMoonlight) elements.themeMoonlight.addEventListener("change", () => setTheme("moonlight"));
